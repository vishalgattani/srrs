...
Usage:
    python -m srrs.main --config CHO --mode D --timesteps 100
    python -m srrs.main --config CHO CHE --mode MC --mc-runs 100 --timesteps 250
    python -m srrs.main --config CHO --mode D --timesteps 100 --plot
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--plot", action="store_true", help="Generate matplotlib PNG output")
...
def run_deterministic(config: Config, params: SimulationParams, args: argparse.Namespace) -> dict:
    ...
    if args.plot:
        out = args.output_dir / "Deterministic" / config.value / f"sim_{config.value}.png"
        plot_simulation(config.value, df, ram.model_dump(), out)
        print(f"  → Plot: {out}")
...
def run_mc(config: Config, params: SimulationParams, args: argparse.Namespace) -> dict:
    ...
    if args.plot:
        out = args.output_dir / "MC" / config.value / f"mc_{config.value}.png"
