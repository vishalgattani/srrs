"""
test_CHO.py — Simulation tests for configuration CHO.
...
def test_cho_simulation_runs():
    """Simulation completes without error for CHO."""
    params = make_params(Config.CHO)
    df, ram = run_simulation(params)
    assert df is not None
    assert ram is not None


def test_cho_dataframe_shape():
...
def test_cho_robot_count_positive():
...
def test_cho_build_quality_in_range():
