"""
test_DHE.py — Simulation tests for configuration DHE.
...
def test_dhe_simulation_runs():
    """Simulation completes without error for DHE."""
    params = make_params(Config.DHE)
    df, ram = run_simulation(params)
    assert df is not None
    assert ram is not None


def test_dhe_dataframe_shape():
...
def test_dhe_robot_count_positive():
...
def test_dhe_build_quality_in_range():
